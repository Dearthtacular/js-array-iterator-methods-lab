...
const inventors = [
  ...
  const inventorFirstAndLastName = inventors.map(function(inventor) {
    return {"first": inventor.first, "last": inventor.last};
  })
  console.log(inventorFirstAndLastName)
  ...
  const inventorObjAda = inventors.find(function(inventor) {
    return inventor.first === 'Ada'
  })

  console.log(inventorObjAda)
  

  // Array.prototype.reduce()
  // 5. How many years did all the inventors live?
  
  const totalLifeSpans = inventors.reduce(function(totalYears, inventor, idx) {
  ...
  // Hint: As a start, consider using the String.prototype.split method to 
  ...
  const dataByType = data.reduce(function(carsObj, type, idx) {
    if (carsObj[type]) {
      carsObj[type] += 1
    } else {
      carsObj[type] = 1
  ...
  const oneIsAtLeast19 = devs.some(function(dev) {
    return ((new Date().getFullYear() - dev.year) >=19)
  })
  
  console.log(oneIsAtLeast19)
  
  // Array.prototype.every()
  // 9. Check if everyone is 19 or older?
  
  const allAreAtLeast19 = devs.every(function(dev) {
    return ((new Date().getFullYear() - dev.year) >=19)
  })

  console.log(allAreAtLeast19);
  ...
  const specificCommentIndex = comments.findIndex(function(comment) {
